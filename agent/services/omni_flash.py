...
def _batch_operation_result(
    operation, project_id: str, model: str, duration_s: int, resolution: str,
    ...
    if installation_id:
        result["_installation_id"] = installation_id
    return result
...
async def generate_omni_flash_text_video(
    ...
    target_inst = preferred_installation or (
        client._extensions[cand_ws].get("installation_id") if cand_ws in client._extensions else None
    )
    try:
        pid = client._batch_project_id(project_id, preferred_installation=target_inst) if target_inst else client._batch_project_id(project_id)
        extra_kwargs = {}
        if target_inst:
    ...
    if target_inst:
        result["_installation_id"] = target_inst
    return result
...
async def _submit_omni_frame_video(
    *,
    ...
    resolution = _validate_resolution(resolution)
    client = get_flow_client()
    cand_ws = getattr(client, "_select_extension", lambda **kw: None)(
        require_token=False,
        preferred_installation=preferred_installation,
        preferred_project_id=project_id or None,
    ) if hasattr(client, "_select_extension") else None
    target_inst = preferred_installation or (
        client._extensions[cand_ws].get("installation_id") if cand_ws in getattr(client, "_extensions", {}) else None
    )
    ...
    try:
        ...
        if target_inst:
            client._remember_operation(operation.operation_id, pid, installation_id=target_inst)
        else:
            client._remember_operation(operation.operation_id, pid)
    except Exception as exc:
        return {"status": 502, "error": f"{type(exc).__name__}: {exc}"}
    return _batch_operation_result(
        operation, pid, batch_model, duration_s, resolution, installation_id=target_inst
    )
