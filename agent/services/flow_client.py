...
class FlowClient:
    ...
    def get_extension_by_installation(self, installation_id: str):
        """Find connection session matching an installation ID."""
        if not installation_id:
            return None
        for ws, session in self._extensions.items():
            if session.get("installation_id") == installation_id:
                return ws
        return None
    ...
    def ws_stats(self) -> dict:
        ...
        return {
            ...
            "authenticated_connections": sum(
                ...
                if session.get("flow_key") or session.get("installation_id")
            ),
    ...
    async def _send(self, method: str, params: dict, timeout: float = 300,
        ...
        for index, extension_ws in enumerate(extension_candidates):
            ...
            try:
                ...
                session["in_flight"] = max(0, session.get("in_flight", 1) - 1)
    ...
    async def create_project(self, project_title: str, tool_name: str = "PINHOLE") -> dict:
        try:
            ...
            if result.get("error"):
                ...
                if pid:
                    logger.info("Project create RPC failed (%s), reusing pinned Flow project %s", result.get("error"), pid[:12])
                    return {"status": 200, "data": {"projectId": pid, "title": project_title}}
            ...
            pid = self.flow_project_id()
            if pid:
                logger.info("Project create exception (%s), reusing pinned Flow project %s", exc, pid[:12])
                return {"status": 200, "data": {"projectId": pid, "title": project_title}}
