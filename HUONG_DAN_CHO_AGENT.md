...
15. **Ưu tiên Batch API (Server tự điều phối):** Hệ thống hỗ trợ Batch ở cả 2 tầng: tầng Agent Studio nội bộ (`/api/requests/batch`) và tầng Client V1 (`/v1/images/generations/batch`, `/v1/videos/generations/batch`). Cả hai đều được backend tự động điều phối hàng đợi (throttle tối đa 5 request song song, 10s cooldown, failover quota giữa các profile tự động). Agent không cần viết script loop thủ công.
