    ...
    for path in sorted(SKILLS_DIR.glob("fk-*.md")):
        ...
        try:
            ...
            if "—" in description:
                ...
                description = description.split("–", 1)[1].strip()
            elif "-" in description and description.lower().startswith(f"fk-{name.lower()}"):
                description = description.split("-", 1)[1].strip()
...
def generate_agents_skills(skills):
    """Create .agents/skills/fk-<name>/SKILL.md for each skill (Agent Skills open standard)."""
    ...
    for skill in skills:
        ...
        skill_dir = AGENTS_SKILLS_DIR / f"fk-{name}"
        ...
        dest.write_text(content, encoding="utf-8")
        count += 1
    print(f"Generated {count} Agent Skills in .agents/skills/fk-*/SKILL.md")
    return count
...
14. **Media IDs are account-scoped** — Google Flow assets cannot be shared across accounts or projects. Never reuse a `media_id` on a different account. When switching accounts (e.g. after quota exhaustion), re-upload the original image to obtain a fresh UUID or supply `image_base64`.
"""
...
def do_interactive(skills):
    ...
    print("  [4] Agent Skills   (.agents/skills/fk-*/SKILL.md - Antigravity, Cursor, etc.)")
    ...
    mapping = {"1": "claude", "2": "gemini", "3": "codex", "4": "agents", "5": "all"}
    ...
    for c in choices:
        ...
        val = mapping[c]
        if val == "all":
            tools = ["claude", "gemini", "codex", "agents"]
            break
        if val not in tools:
            tools.append(val)
...
def do_clean():
    ...
    if AGENTS_SKILLS_DIR.exists():
        for skill_dir in AGENTS_SKILLS_DIR.glob("fk-*"):
            if skill_dir.is_dir():
...
def main():
    ...
    parser.add_argument(
        "--tool",
        choices=["claude", "gemini", "codex", "agents", "all"],
