# FlowKit — Tham khảo lịch sử nội bộ agent, không dùng cho client

Các ví dụ video trong tài liệu này là contract cũ, không phải cam kết hỗ trợ
cho client hiện tại. Hướng dẫn client: CLIENT_V1.md.

Tài liệu tổng quan endpoint FlowKit Server. **Client Backend** là lớp tương thích cho client, không phải toàn bộ workflow FlowKit hay cam kết parity 1:1 với một server khác. Xem [contract v1 và giới hạn thực tế](CLIENT_V1.md) trước khi tích hợp; schema đầy đủ nằm tại `/openapi.json` và `/docs`.
...
*Có thể bỏ `project_id` và gửi ảnh Base64 để server chọn profile. UUID có sẵn phải có ownership hợp lệ. Xem [routing và execution logs](EXECUTION_LOGS.md).*
...
`generation_type` là tên ưu tiên; `type` vẫn được hỗ trợ. `model` ưu tiên hơn `mode`, `model_family`, `quality`. Khả năng thực tế phụ thuộc transport/provider; không suy ra model upstream từ tên alias client.
...
```
**Request Body (Reference-to-Video - R2V):**

Lưu ý: batch transport hiện trả lỗi unsupported cho R2V. `FLOW_ALLOW_DEGRADED=1` chỉ hạ xuống I2V từ ảnh reference đầu tiên, không phải R2V đầy đủ. Ví dụ này mô tả contract, không cam kết provider hỗ trợ.
```json
{
...
| `GET` | `/v1/jobs/{job_id}/executions` | Lịch sử gọi provider; xem [EXECUTION_LOGS.md](EXECUTION_LOGS.md) |

**Kết quả khi hoàn thành (`status == "complete"`):**

Ví dụ minh họa cấu trúc; `duration_seconds`, `thumbnail_url`, kích thước có thể `null`
vì v1 hiện không điền các metadata này từ provider.
```json
{
  "jobs": [
    {
...
- Ghép video bằng workflow `/fk-concat` / ffmpeg; hiện không có route `POST /api/videos/{id}/concat`.
...
- `POST /api/flow/check-status`: Tra cứu trạng thái video từ Google Flow RPC.

### 2.6. Lồng tiếng & TTS
- `POST /api/tts/generate`: Sinh file âm thanh WAV từ văn bản lồng tiếng (`narrator_text`).
- `POST /api/tts/templates`: Tạo mẫu giọng đọc mới.
...
- `GET /api/models`: Cấu hình model ảnh/video.
- `GET /api/providers`: Cấu hình CLI provider cho review.
- `GET /api/active-project`: Lấy project đang hoạt động.
